...
public class SolitareDisplay extends JComponent implements MouseListener {
	...
	public void paintComponent(Graphics g) {
		...
		for (int i = 0; i < pile.size(); i++) {
			drawCard(g, pile.get(i), spacing + (CARD_WIDTH + spacing) * Solitare.pileToInt(pile),
					CARD_HEIGHT + 2 * spacing + offset);
			if (selectedRow == 1 && selectedCol == Solitare.pileToInt(pile) && i == pile.size() - 1) {
				drawBorder(g, spacing + (CARD_WIDTH + spacing) * Solitare.pileToInt(pile),
						CARD_HEIGHT + 2 * spacing + offset);
			}
